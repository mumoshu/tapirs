...
```
 Current                                                       Future
────────────────────────────────────────────────────────────────────────────
  [Kubernetes Testbed] ──> [Kind Testbed] ──> [Kubernetes Operator]
                                                     │
                                              [Observability]
                                                     │
                                                [Benchmark]
...
| Kubernetes Operator | TAPIRCluster CRD for declarative cluster management — discovery, shard-manager, node pools, per-shard configuration. Phase-based bootstrap with static membership, dynamic scaling via shard-manager join. See `kubernetes/operator/` | Done |
