...
**How tapirs handles it:** **Bug: piggybacking not implemented.** `invoke_inconsistent` sends Finalize via `do_send` (spawned task, fire-and-forget) — it does NOT implement the paper's piggybacking. Source: `ir/client.rs:411-417`. This is a linearizability bug: there is no ordering guarantee between the fire-and-forget Finalize and the client's next operation. Tests work around the bug with `T::yield_now()` + `T::sleep(Duration::from_millis(1))` after `commit()` to give the Finalize task time to run. On real networks, the bug is masked because the Finalize is typically delivered before the next operation completes its round trip, but this is not guaranteed. The fix is to implement piggybacking: buffer pending Finalizes in the IR client and attach them to the next Propose message sent to overlapping replicas, ensuring the replica processes the Finalize before the new operation. Once piggybacking is implemented, the `sleep(1ms)` workaround in tests should be removed — it's artificial delay masking the bug. The `yield_now()` can optionally stay as a zero-cost fairness yield, but is no longer required for correctness. This would provide the linearizability guarantee the paper specifies. `invoke_inconsistent_with_result` (used by QuorumRead) waits for f+1 Finalize replies, so the returned result reflects executed state — no piggybacking issue for this variant. Source: `ir/client.rs:570-592`.
