...
**Proposed extension — bounded-history compaction:** per-key MVCC growth could be capped by letting `tapictl compact shard` take a retention watermark and drop pre-watermark versions beyond a single per-key snapshot. This is not implemented; for the design, constraints, and correctness analysis see [roadmap/bounded-history-compaction](../roadmap/bounded-history-compaction.md).
