...
pub async fn backup_cluster(
    ...
    tracing::info!(
        count = shard_to_nodes.len(),
        shards = ?shard_to_nodes.keys().collect::<Vec<_>>(),
        "found unique shards",
    );
    ...
    for (shard_id, nodes) in &shard_to_nodes {
        ...
        tracing::info!(shard_id, "forcing view change");
        ...
        tracing::info!(shard_id, "waiting for view change to settle");
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;

        tracing::info!(shard_id, "backing up shard");
        ...
        tracing::info!(shard_id, path, "backup written");
    }
