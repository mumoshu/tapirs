...
fn recovery_vlog_and_manifest_survive_reopen() {
    ...
    let inner = UnifiedStore::<String, MemoryIo>::open(path).unwrap();
...
fn recovery_sealed_segments_persist() {
    ...
    {
        let inner =
            UnifiedStore::<String, MemoryIo>::open_with_options(path.clone(), 1024).unwrap();
        let mut store = UnifiedMvccBackend::<String, String, MemoryIo>::new(inner);
    ...
    let inner =
        UnifiedStore::<String, MemoryIo>::open_with_options(path, 1024).unwrap();
    let store = UnifiedMvccBackend::<String, String, MemoryIo>::new(inner);
