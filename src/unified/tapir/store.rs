use crate::mvcc::disk::error::StorageError;
use crate::mvcc::disk::disk_io::{DiskIo, OpenFlags};
use crate::occ::TransactionId as OccTransactionId;
use crate::occ::TransactionId;
use crate::tapir::{Key, LeaderRecordDelta, Timestamp, Value};
use crate::tapirstore::{MinPrepareTimes, RecordDeltaDuringView, TransactionLog};
use crate::unified::tapir::CachedPrepare;
use crate::unified::types::{
    UnifiedLsmEntry, UnifiedVlogPrepareValuePtr, UnifiedVlogPtr, ValueLocation, VlogSegmentMeta,
};
use crate::unified::wisckeylsm::manifest::UnifiedManifest;
use crate::unified::wisckeylsm::prepare_cache::PrepareCache;
use crate::unified::wisckeylsm::unified_memtable::UnifiedMemtable;
use crate::unified::wisckeylsm::vlog::UnifiedVlogSegment;
use std::cell::{Cell, RefCell};
use std::collections::BTreeMap;
use std::path::{Path, PathBuf};
use std::sync::Arc;

pub(crate) struct TapirState<K: Ord, V, IO: DiskIo> {
    unified_memtable: UnifiedMemtable<K>,
    prepare_registry: BTreeMap<OccTransactionId, Arc<CachedPrepare<K, V>>>,
    prepare_cache: RefCell<PrepareCache<CachedPrepare<K, V>>>,
    committed_txn_vlog_index: BTreeMap<OccTransactionId, UnifiedVlogPtr>,
    vlog_read_count: Cell<u64>,
    txlog: TransactionLog,
    min_prepare: MinPrepareTimes,
    cdc: RecordDeltaDuringView<K, V>,
    active_vlog: UnifiedVlogSegment<IO>,
    sealed_vlog_segments: BTreeMap<u64, UnifiedVlogSegment<IO>>,
    manifest: UnifiedManifest,
    base_dir: PathBuf,
    io_flags: OpenFlags,
    current_view_entry_count: u32,
}

fn new_store_state<K: Ord + Clone, V, IO: DiskIo>(
    prepare_cache_capacity: usize,
    active_vlog: UnifiedVlogSegment<IO>,
    sealed_vlog_segments: BTreeMap<u64, UnifiedVlogSegment<IO>>,
    manifest: UnifiedManifest,
    base_dir: PathBuf,
    io_flags: OpenFlags,
) -> TapirState<K, V, IO> {
    TapirState {
        unified_memtable: UnifiedMemtable::new(),
        prepare_registry: BTreeMap::new(),
        prepare_cache: RefCell::new(PrepareCache::new(prepare_cache_capacity)),
        committed_txn_vlog_index: BTreeMap::new(),
        vlog_read_count: Cell::new(0),
        txlog: TransactionLog::new(),
        min_prepare: MinPrepareTimes::new(),
        cdc: RecordDeltaDuringView::new(),
        active_vlog,
        sealed_vlog_segments,
        manifest,
        base_dir,
        io_flags,
        current_view_entry_count: 0,
    }
}

    impl<K: Ord + Clone, V, IO: DiskIo> TapirState<K, V, IO> {
    pub(crate) fn vlog_read_count(&self) -> u64 {
        self.vlog_read_count.get()
    }

    pub(crate) fn increment_vlog_read_count(&self) {
        self.vlog_read_count.set(self.vlog_read_count.get() + 1);
    }

    pub(crate) fn unified_memtable(&self) -> &UnifiedMemtable<K> {
        &self.unified_memtable
    }

    pub(crate) fn unified_memtable_mut(&mut self) -> &mut UnifiedMemtable<K> {
        &mut self.unified_memtable
    }

    pub(crate) fn register_prepare_raw(
        &mut self,
        txn_id: OccTransactionId,
        prepare: Arc<CachedPrepare<K, V>>,
    ) {
        self.prepare_registry.insert(txn_id, prepare);
    }

    pub(crate) fn unregister_prepare(&mut self, txn_id: &OccTransactionId) {
        self.prepare_registry.remove(txn_id);
    }

    pub(crate) fn lookup_prepare(&self, txn_id: &OccTransactionId) -> Option<Arc<CachedPrepare<K, V>>> {
        self.prepare_registry.get(txn_id).cloned()
    }

    pub(crate) fn lookup_prepare_value_ref(
        &self,
        txn_id: &OccTransactionId,
        write_index: u16,
    ) -> Option<(K, Option<V>)>
    where
        K: Clone,
        V: Clone,
    {
        self.prepare_registry
            .get(txn_id)
            .and_then(|p| p.write_set.get(write_index as usize))
            .cloned()
    }

    pub(crate) fn resolve_in_memory_ref(
        &self,
        txn_id: &OccTransactionId,
        write_index: u16,
    ) -> Option<&(K, Option<V>)> {
        self.prepare_registry
            .get(txn_id)
            .and_then(|p| p.write_set.get(write_index as usize))
    }

    #[cfg(test)]
    pub(crate) fn has_prepare_write_conflict(&self, key: &K) -> bool {
        self.prepare_registry
            .values()
            .flat_map(|p| p.write_set.iter())
            .any(|(k, _)| k == key)
    }

    #[cfg(test)]
    pub(crate) fn contains_prepare(&self, txn_id: &OccTransactionId) -> bool {
        self.prepare_registry.contains_key(txn_id)
    }

    pub(crate) fn clear_prepare_registry(&mut self) {
        self.prepare_registry.clear();
    }

    pub(crate) fn min_prepared_commit_time(&self) -> Option<u64> {
        self.prepare_registry
            .values()
            .map(|p| p.commit_ts.time)
            .min()
    }

    pub(crate) fn index_committed_txn_ptr(&mut self, txn_id: OccTransactionId, ptr: UnifiedVlogPtr) {
        self.committed_txn_vlog_index.insert(txn_id, ptr);
    }

    pub(crate) fn lookup_committed_txn_ptr(&self, txn_id: &OccTransactionId) -> Option<UnifiedVlogPtr> {
        self.committed_txn_vlog_index.get(txn_id).copied()
    }

    pub(crate) fn prepare_cache_get(
        &self,
        segment_id: u64,
        offset: u64,
    ) -> Option<Arc<CachedPrepare<K, V>>> {
        self.prepare_cache.borrow_mut().get(segment_id, offset)
    }

    pub(crate) fn prepare_cache_insert(
        &self,
        segment_id: u64,
        offset: u64,
        prepare: Arc<CachedPrepare<K, V>>,
    ) {
        self.prepare_cache
            .borrow_mut()
            .insert(segment_id, offset, prepare);
    }

    pub(crate) fn txn_log_get(&self, id: &OccTransactionId) -> Option<(Timestamp, bool)> {
        self.txlog.txn_log_get(id)
    }

    pub(crate) fn txn_log_insert(
        &mut self,
        id: OccTransactionId,
        ts: Timestamp,
        committed: bool,
    ) -> Option<(Timestamp, bool)> {
        self.txlog.txn_log_insert(id, ts, committed)
    }

    pub(crate) fn txn_log_contains(&self, id: &OccTransactionId) -> bool {
        self.txlog.txn_log_contains(id)
    }

    pub(crate) fn txn_log_len(&self) -> usize {
        self.txlog.txn_log_len()
    }

    pub(crate) fn raise_min_prepare_time(&mut self, time: u64) -> u64 {
        self.min_prepare.raise(time, self.min_prepared_commit_time())
    }

    pub(crate) fn finalize_min_prepare_time(&mut self, time: u64) {
        self.min_prepare.finalize(time)
    }

    pub(crate) fn sync_min_prepare_time(&mut self, time: u64) {
        self.min_prepare.sync(time)
    }

    pub(crate) fn reset_min_prepare_time_to_finalized(&mut self) {
        self.min_prepare.reset_to_finalized()
    }

    pub(crate) fn record_cdc_delta(&mut self, base_view: u64, delta: LeaderRecordDelta<K, V>) {
        self.cdc.record_cdc_delta(base_view, delta);
    }

    pub(crate) fn cdc_deltas_from(&self, from_view: u64) -> Vec<LeaderRecordDelta<K, V>>
    where
        V: Clone,
    {
        self.cdc.cdc_deltas_from(from_view)
    }

    pub(crate) fn cdc_max_view(&self) -> Option<u64> {
        self.cdc.cdc_max_view()
    }

    pub(crate) fn convert_in_memory_to_on_disk(&mut self) {
        let committed_index = self.committed_txn_vlog_index.clone();
        self.unified_memtable
            .convert_in_memory_to_on_disk(&committed_index);
    }

    fn active_or_sealed_segment(&self, segment_id: u64) -> Option<&UnifiedVlogSegment<IO>> {
        self.sealed_vlog_segments
            .get(&segment_id)
            .or(if self.active_vlog.id == segment_id {
                Some(&self.active_vlog)
            } else {
                None
            })
    }

    fn append_committed_txn_to_active(
        &mut self,
        txn_id: OccTransactionId,
        commit: Timestamp,
        read_set: &[(K, Timestamp)],
        write_set: &[(K, Option<V>)],
        scan_set: &[(K, K, Timestamp)],
    ) -> Result<UnifiedVlogPtr, StorageError>
    where
        K: serde::Serialize,
        V: serde::Serialize,
    {
        let ptr = self
            .active_vlog
            .append_committed_txn(txn_id, commit, read_set, write_set, scan_set)?;
        self.current_view_entry_count += 1;
        Ok(ptr)
    }

    pub(crate) fn seal_current_view(&mut self, min_view_vlog_size: u64) -> Result<(), StorageError> {
        self.active_vlog.sync()?;
        self.active_vlog
            .finish_view(self.current_view_entry_count);

        let segment_size = self.active_vlog.write_offset();
        if segment_size >= min_view_vlog_size {
            let sealed_id = self.active_vlog.id;
            let sealed_path = self.active_vlog.path().clone();
            let sealed_views = self.active_vlog.views.clone();
            let sealed_size = self.active_vlog.write_offset();

            self.manifest.sealed_vlog_segments.push(VlogSegmentMeta {
                segment_id: sealed_id,
                path: sealed_path.clone(),
                views: sealed_views.clone(),
                total_size: sealed_size,
            });

            let old_active = std::mem::replace(
                &mut self.active_vlog,
                {
                    let new_id = self.manifest.next_segment_id;
                    self.manifest.next_segment_id += 1;
                    let new_path = self
                        .base_dir
                        .join(format!("vlog_seg_{new_id:04}.dat"));
                    UnifiedVlogSegment::<IO>::open(new_id, new_path, self.io_flags)?
                },
            );
            self.sealed_vlog_segments.insert(
                sealed_id,
                UnifiedVlogSegment::<IO>::open_at(
                    sealed_id,
                    sealed_path,
                    sealed_size,
                    sealed_views,
                    self.io_flags,
                )?,
            );
            old_active.close();
        }

        self.manifest.current_view += 1;
        self.manifest.active_segment_id = self.active_vlog.id;
        self.manifest.active_write_offset = self.active_vlog.write_offset();
        self.manifest.save::<IO>(&self.base_dir)?;
        self.active_vlog.start_view(self.manifest.current_view);

        finalize_after_seal(self);
        self.current_view_entry_count = 0;
        Ok(())
    }
}

const DEFAULT_PREPARE_CACHE_CAPACITY: usize = 1024;
const RAW_ENTRY_OVERHEAD: u32 = 25;
const TAPIR_COMMITTED_TXN_ENTRY_TYPE: u8 = 0x80;

fn recover_segment<K: Key, V: Value, IO: DiskIo>(
    state: &mut TapirState<K, V, IO>,
    segment: &UnifiedVlogSegment<IO>,
) -> Result<(), StorageError> {
    for (offset, raw) in segment.iter_raw_entries()? {
        if raw.entry_type != TAPIR_COMMITTED_TXN_ENTRY_TYPE {
            continue;
        }

        let prepared = crate::unified::tapir::vlog_codec::deserialize_committed_txn_payload::<K, V>(&raw.payload)?;
        let txn_id = prepared.transaction_id;
        let commit_ts = prepared.commit_ts;
        let read_set = prepared.read_set;
        let write_set = prepared.write_set;

        let txn_ptr = UnifiedVlogPtr {
            segment_id: segment.id,
            offset,
            length: RAW_ENTRY_OVERHEAD + raw.payload.len() as u32,
        };
        state.index_committed_txn_ptr(txn_id, txn_ptr);

        for (i, (key, value)) in write_set.iter().enumerate() {
            let value_ref = if value.is_some() {
                Some(ValueLocation::OnDisk(UnifiedVlogPrepareValuePtr {
                    txn_ptr,
                    write_index: i as u16,
                }))
            } else {
                None
            };
            state.unified_memtable_mut().insert(
                key.clone(),
                commit_ts,
                UnifiedLsmEntry {
                    value_ref,
                    last_read_ts: None,
                },
            );
        }

        for (key, read) in &read_set {
            state.unified_memtable_mut().update_last_read(key, *read, commit_ts.time);
        }
    }

    Ok(())
}

pub(crate) fn open<K: Key, V: Value, IO: DiskIo>(
    base_dir: &Path,
    io_flags: OpenFlags,
) -> Result<TapirState<K, V, IO>, StorageError> {
    IO::create_dir_all(base_dir)?;

    let manifest = UnifiedManifest::load::<IO>(base_dir)?.unwrap_or_else(UnifiedManifest::new);

    let mut sealed_vlog_segments = BTreeMap::new();
    for seg_meta in &manifest.sealed_vlog_segments {
        let seg = UnifiedVlogSegment::<IO>::open_at(
            seg_meta.segment_id,
            seg_meta.path.clone(),
            seg_meta.total_size,
            seg_meta.views.clone(),
            io_flags,
        )?;
        sealed_vlog_segments.insert(seg_meta.segment_id, seg);
    }

    let active_path = base_dir.join(format!("vlog_seg_{:04}.dat", manifest.active_segment_id));
    let mut active_vlog = UnifiedVlogSegment::<IO>::open_at(
        manifest.active_segment_id,
        active_path,
        manifest.active_write_offset,
        Vec::new(),
        io_flags,
    )?;
    active_vlog.start_view(manifest.current_view);

    let mut state = new_store_state::<K, V, IO>(
        DEFAULT_PREPARE_CACHE_CAPACITY,
        active_vlog,
        sealed_vlog_segments,
        manifest,
        base_dir.to_path_buf(),
        io_flags,
    );

    for seg_meta in state.manifest.sealed_vlog_segments.clone() {
        let seg = UnifiedVlogSegment::<IO>::open_at(
            seg_meta.segment_id,
            seg_meta.path,
            seg_meta.total_size,
            seg_meta.views,
            state.io_flags,
        )?;
        recover_segment(&mut state, &seg)?;
        seg.close();
    }

    let active_for_recover = UnifiedVlogSegment::<IO>::open_at(
        state.manifest.active_segment_id,
        state
            .base_dir
            .join(format!("vlog_seg_{:04}.dat", state.manifest.active_segment_id)),
        state.manifest.active_write_offset,
        Vec::new(),
        state.io_flags,
    )?;
    recover_segment(&mut state, &active_for_recover)?;
    active_for_recover.close();

    Ok(state)
}

impl<
        K: Key + serde::Serialize + serde::de::DeserializeOwned,
        V: Value + serde::Serialize + serde::de::DeserializeOwned,
        IO: DiskIo,
    > TapirStoreBackend<K, V> for TapirState<K, V, IO>
{
    fn tapir_uncommitted_get(&self, key: &K) -> Result<(Option<V>, Timestamp), StorageError> {
        if let Some((ck, entry)) = self.unified_memtable().get_latest(key) {
            let ts = ck.timestamp.0;
            let value = resolve_value(self, entry)?;
            return Ok((value, ts));
        }
        Ok((None, Timestamp::default()))
    }

    fn tapir_uncommitted_get_at(&self, key: &K, ts: Timestamp) -> Result<(Option<V>, Timestamp), StorageError> {
        if let Some((ck, entry)) = self.unified_memtable().get_at(key, ts) {
            let write_ts = ck.timestamp.0;
            let value = resolve_value(self, entry)?;
            return Ok((value, write_ts));
        }
        Ok((None, Timestamp::default()))
    }

    fn tapir_uncommitted_scan(
        &self,
        start: &K,
        end: &K,
        ts: Timestamp,
    ) -> Result<Vec<(K, Option<V>, Timestamp)>, StorageError> {
        let mut out = Vec::new();
        for (ck, entry) in self.unified_memtable().scan(start, end, ts) {
            let value = resolve_value(self, entry)?;
            out.push((ck.key.clone(), value, ck.timestamp.0));
        }
        Ok(out)
    }

    fn tapir_txn_log_get(&self, id: &TransactionId) -> Option<(Timestamp, bool)> {
        self.txn_log_get(id)
    }

    fn tapir_txn_log_insert(
        &mut self,
        id: TransactionId,
        ts: Timestamp,
        committed: bool,
    ) -> Option<(Timestamp, bool)> {
        self.txn_log_insert(id, ts, committed)
    }

    fn tapir_txn_log_contains(&self, id: &TransactionId) -> bool {
        self.txn_log_contains(id)
    }

    fn tapir_txn_log_len(&self) -> usize {
        self.txn_log_len()
    }

    fn tapir_raise_min_prepare_time(&mut self, time: u64) -> u64 {
        self.raise_min_prepare_time(time)
    }

    fn tapir_finalize_min_prepare_time(&mut self, time: u64) {
        self.finalize_min_prepare_time(time)
    }

    fn tapir_sync_min_prepare_time(&mut self, time: u64) {
        self.sync_min_prepare_time(time)
    }

    fn tapir_reset_min_prepare_time_to_finalized(&mut self) {
        self.reset_min_prepare_time_to_finalized()
    }

    fn tapir_record_cdc_delta(&mut self, base_view: u64, delta: LeaderRecordDelta<K, V>) {
        self.record_cdc_delta(base_view, delta)
    }

    fn tapir_cdc_deltas_from(&self, from_view: u64) -> Vec<LeaderRecordDelta<K, V>> {
        self.cdc_deltas_from(from_view)
    }

    fn tapir_cdc_max_view(&self) -> Option<u64> {
        self.cdc_max_view()
    }

    fn tapir_append_committed_txn(
        &mut self,
        txn_id: OccTransactionId,
        commit: Timestamp,
        read_set: &[(K, Timestamp)],
        write_set: &[(K, Option<V>)],
        scan_set: &[(K, K, Timestamp)],
    ) -> Result<UnifiedVlogPtr, StorageError> {
        self.append_committed_txn_to_active(txn_id, commit, read_set, write_set, scan_set)
    }

    fn tapir_index_committed_txn_ptr(&mut self, txn_id: OccTransactionId, txn_ptr: UnifiedVlogPtr) {
        self.index_committed_txn_ptr(txn_id, txn_ptr)
    }

    fn tapir_register_prepare_raw(
        &mut self,
        txn_id: OccTransactionId,
        prepare: Arc<CachedPrepare<K, V>>,
    ) {
        self.register_prepare_raw(txn_id, prepare)
    }

    fn tapir_unregister_prepare(&mut self, txn_id: &OccTransactionId) {
        self.unregister_prepare(txn_id)
    }

    fn tapir_lookup_prepare(&self, txn_id: &OccTransactionId) -> Option<Arc<CachedPrepare<K, V>>> {
        self.lookup_prepare(txn_id)
    }

    fn tapir_lookup_prepare_value_ref(
        &self,
        txn_id: &OccTransactionId,
        write_index: u16,
    ) -> Option<(K, Option<V>)> {
        self.lookup_prepare_value_ref(txn_id, write_index)
    }

    fn tapir_lookup_committed_txn_ptr(&self, txn_id: &OccTransactionId) -> Option<UnifiedVlogPtr> {
        self.lookup_committed_txn_ptr(txn_id)
    }

    fn tapir_prepare_cache_get(
        &self,
        segment_id: u64,
        offset: u64,
    ) -> Option<Arc<CachedPrepare<K, V>>> {
        self.prepare_cache_get(segment_id, offset)
    }

    fn tapir_prepare_cache_insert(
        &self,
        segment_id: u64,
        offset: u64,
        prepare: Arc<CachedPrepare<K, V>>,
    ) {
        self.prepare_cache_insert(segment_id, offset, prepare)
    }

    fn tapir_increment_vlog_read_count(&self) {
        self.increment_vlog_read_count()
    }

    fn tapir_read_committed_txn(
        &self,
        txn_ptr: &UnifiedVlogPtr,
    ) -> Result<CachedPrepare<K, V>, StorageError> {
        let segment_id = txn_ptr.segment_id;
        let segment = self
            .active_or_sealed_segment(segment_id)
            .ok_or_else(|| StorageError::Codec(format!(
                "VLog segment {segment_id} not found for prepare resolution"
            )))?;

        segment.read_committed_txn(txn_ptr)
    }

    fn tapir_insert_committed_write(
        &mut self,
        key: K,
        commit: Timestamp,
        value_ref: Option<ValueLocation>,
    ) {
        self.unified_memtable_mut().insert(
            key,
            commit,
            UnifiedLsmEntry {
                value_ref,
                last_read_ts: None,
            },
        );
    }

    fn tapir_update_last_read(&mut self, key: &K, read: Timestamp, commit_time: u64) {
        self.unified_memtable_mut()
            .update_last_read(key, read, commit_time);
    }
}

pub(crate) trait TapirStoreBackend<K, V> {
    fn tapir_uncommitted_get(&self, key: &K) -> Result<(Option<V>, Timestamp), StorageError>;
    fn tapir_uncommitted_get_at(
        &self,
        key: &K,
        ts: Timestamp,
    ) -> Result<(Option<V>, Timestamp), StorageError>;
    fn tapir_uncommitted_scan(
        &self,
        start: &K,
        end: &K,
        ts: Timestamp,
    ) -> Result<Vec<(K, Option<V>, Timestamp)>, StorageError>;

    fn tapir_txn_log_get(&self, id: &TransactionId) -> Option<(Timestamp, bool)>;
    fn tapir_txn_log_insert(
        &mut self,
        id: TransactionId,
        ts: Timestamp,
        committed: bool,
    ) -> Option<(Timestamp, bool)>;
    fn tapir_txn_log_contains(&self, id: &TransactionId) -> bool;
    fn tapir_txn_log_len(&self) -> usize;

    fn tapir_raise_min_prepare_time(&mut self, time: u64) -> u64;
    fn tapir_finalize_min_prepare_time(&mut self, time: u64);
    fn tapir_sync_min_prepare_time(&mut self, time: u64);
    fn tapir_reset_min_prepare_time_to_finalized(&mut self);

    fn tapir_record_cdc_delta(&mut self, base_view: u64, delta: LeaderRecordDelta<K, V>);
    fn tapir_cdc_deltas_from(&self, from_view: u64) -> Vec<LeaderRecordDelta<K, V>>;
    fn tapir_cdc_max_view(&self) -> Option<u64>;

    fn tapir_append_committed_txn(
        &mut self,
        txn_id: OccTransactionId,
        commit: Timestamp,
        read_set: &[(K, Timestamp)],
        write_set: &[(K, Option<V>)],
        scan_set: &[(K, K, Timestamp)],
    ) -> Result<UnifiedVlogPtr, StorageError>;
    fn tapir_index_committed_txn_ptr(&mut self, txn_id: OccTransactionId, txn_ptr: UnifiedVlogPtr);

    fn tapir_register_prepare_raw(
        &mut self,
        txn_id: OccTransactionId,
        prepare: Arc<CachedPrepare<K, V>>,
    );
    fn tapir_unregister_prepare(&mut self, txn_id: &OccTransactionId);
    fn tapir_lookup_prepare(&self, txn_id: &OccTransactionId) -> Option<Arc<CachedPrepare<K, V>>>;
    fn tapir_lookup_prepare_value_ref(
        &self,
        txn_id: &OccTransactionId,
        write_index: u16,
    ) -> Option<(K, Option<V>)>;
    fn tapir_lookup_committed_txn_ptr(&self, txn_id: &OccTransactionId) -> Option<crate::unified::types::UnifiedVlogPtr>;
    fn tapir_prepare_cache_get(
        &self,
        segment_id: u64,
        offset: u64,
    ) -> Option<Arc<CachedPrepare<K, V>>>;
    fn tapir_prepare_cache_insert(
        &self,
        segment_id: u64,
        offset: u64,
        prepare: Arc<CachedPrepare<K, V>>,
    );
    fn tapir_increment_vlog_read_count(&self);
    fn tapir_read_committed_txn(
        &self,
        txn_ptr: &UnifiedVlogPtr,
    ) -> Result<CachedPrepare<K, V>, StorageError>;
    fn tapir_insert_committed_write(
        &mut self,
        key: K,
        commit: Timestamp,
        value_ref: Option<ValueLocation>,
    );
    fn tapir_update_last_read(&mut self, key: &K, read: Timestamp, commit_time: u64);
}

pub(crate) fn commit_transaction_data<
    K: serde::Serialize + Clone + serde::de::DeserializeOwned,
    V: serde::Serialize + Clone + serde::de::DeserializeOwned,
    T: TapirStoreBackend<K, V>,
>(
    store: &mut T,
    txn_id: OccTransactionId,
    read_set: &[(K, Timestamp)],
    write_set: &[(K, Option<V>)],
    scan_set: &[(K, K, Timestamp)],
    commit: Timestamp,
) -> Result<(), StorageError> {
    let txn_ptr = store.tapir_append_committed_txn(txn_id, commit, read_set, write_set, scan_set)?;
    store.tapir_index_committed_txn_ptr(txn_id, txn_ptr);

    for (i, (key, value)) in write_set.iter().enumerate() {
        let value_ref = if value.is_some() {
            Some(ValueLocation::OnDisk(UnifiedVlogPrepareValuePtr {
                txn_ptr,
                write_index: i as u16,
            }))
        } else {
            None
        };

        store.tapir_insert_committed_write(key.clone(), commit, value_ref);
    }

    for (key, read) in read_set {
        store.tapir_update_last_read(key, *read, commit.time);
    }

    Ok(())
}

pub(crate) fn finalize_after_seal<K: Ord + Clone, V, IO: DiskIo>(state: &mut TapirState<K, V, IO>) {
    state.convert_in_memory_to_on_disk();
    state.clear_prepare_registry();
}

pub(crate) fn register_prepare<K: Key, V: Value + Clone, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    txn_id: OccTransactionId,
    transaction: &crate::occ::Transaction<K, V, Timestamp>,
    commit_ts: Timestamp,
) {
    let shard = crate::tapir::ShardNumber(0);

    let read_set: Vec<(K, Timestamp)> = transaction
        .shard_read_set(shard)
        .map(|(k, ts)| (k.clone(), ts))
        .collect();

    let write_set: Vec<(K, Option<V>)> = transaction
        .shard_write_set(shard)
        .map(|(k, v)| (k.clone(), v.clone()))
        .collect();

    let scan_set: Vec<(K, K, Timestamp)> = transaction
        .shard_scan_set(shard)
        .map(|entry| {
            (
                entry.start_key.clone(),
                entry.end_key.clone(),
                entry.timestamp,
            )
        })
        .collect();

    let prepare = Arc::new(CachedPrepare {
        transaction_id: txn_id,
        commit_ts,
        read_set,
        write_set,
        scan_set,
    });

    store.tapir_register_prepare_raw(txn_id, prepare);
}

pub(crate) fn unregister_prepare<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    txn_id: &OccTransactionId,
) {
    store.tapir_unregister_prepare(txn_id);
}

pub(crate) fn commit_prepared<
    K: serde::Serialize + Clone + serde::de::DeserializeOwned,
    V: serde::Serialize + Clone + serde::de::DeserializeOwned,
    T: TapirStoreBackend<K, V>,
>(
    store: &mut T,
    txn_id: OccTransactionId,
    commit: Timestamp,
) -> Result<(), StorageError> {
    let prepare = store.tapir_lookup_prepare(&txn_id);
    let cross_view_ptr = store.tapir_lookup_committed_txn_ptr(&txn_id);

    if let Some(prepare) = prepare {
        return commit_transaction_data(
            store,
            txn_id,
            &prepare.read_set,
            &prepare.write_set,
            &prepare.scan_set,
            commit,
        );
    }

    if let Some(vlog_ptr) = cross_view_ptr {
        let cached = resolve_on_disk(store, &UnifiedVlogPrepareValuePtr {
            txn_ptr: vlog_ptr,
            write_index: 0,
        })?;

        for (i, (key, value)) in cached.write_set.iter().enumerate() {
            let value_ref = if value.is_some() {
                Some(ValueLocation::OnDisk(UnifiedVlogPrepareValuePtr {
                    txn_ptr: vlog_ptr,
                    write_index: i as u16,
                }))
            } else {
                None
            };
            store.tapir_insert_committed_write(key.clone(), commit, value_ref);
        }

        for (key, read) in &cached.read_set {
            store.tapir_update_last_read(key, *read, commit.time);
        }
        return Ok(());
    }

    Err(StorageError::PrepareNotFound {
        client_id: txn_id.client_id.0,
        txn_number: txn_id.number,
    })
}

pub(crate) fn resolve_in_memory<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    txn_id: &OccTransactionId,
    write_index: u16,
) -> Option<(K, Option<V>)> {
    store.tapir_lookup_prepare_value_ref(txn_id, write_index)
}

pub(crate) fn resolve_on_disk<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    ptr: &UnifiedVlogPrepareValuePtr,
) -> Result<Arc<CachedPrepare<K, V>>, StorageError> {
    let segment_id = ptr.txn_ptr.segment_id;
    let offset = ptr.txn_ptr.offset;

    if let Some(cached) = store.tapir_prepare_cache_get(segment_id, offset) {
        return Ok(cached);
    }

    store.tapir_increment_vlog_read_count();
    let prepare = store.tapir_read_committed_txn(&ptr.txn_ptr)?;
    let cached = Arc::new(prepare);
    store.tapir_prepare_cache_insert(segment_id, offset, cached.clone());
    Ok(cached)
}

pub(crate) fn resolve_value<
    K: serde::de::DeserializeOwned,
    V: Clone + serde::de::DeserializeOwned,
    T: TapirStoreBackend<K, V>,
>(
    store: &T,
    entry: &UnifiedLsmEntry,
) -> Result<Option<V>, StorageError> {
    match &entry.value_ref {
        None => Ok(None),
        Some(ValueLocation::InMemory { txn_id, write_index }) => {
            match resolve_in_memory(store, txn_id, *write_index) {
                Some((_key, value)) => Ok(value.clone()),
                None => Ok(None),
            }
        }
        Some(ValueLocation::OnDisk(ptr)) => {
            let cached = resolve_on_disk(store, ptr)?;
            if let Some((_key, value)) = cached.write_set.get(ptr.write_index as usize) {
                Ok(value.clone())
            } else {
                Ok(None)
            }
        }
    }
}

pub(crate) fn do_uncommitted_get<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    key: &K,
) -> Result<(Option<V>, Timestamp), StorageError> {
    store.tapir_uncommitted_get(key)
}

pub(crate) fn do_uncommitted_get_at<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    key: &K,
    ts: Timestamp,
) -> Result<(Option<V>, Timestamp), StorageError> {
    store.tapir_uncommitted_get_at(key, ts)
}

pub(crate) fn do_uncommitted_scan<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    start: &K,
    end: &K,
    ts: Timestamp,
) -> Result<Vec<(K, Option<V>, Timestamp)>, StorageError> {
    store.tapir_uncommitted_scan(start, end, ts)
}

pub(crate) fn txn_log_get<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    id: &TransactionId,
) -> Option<(Timestamp, bool)> {
    store.tapir_txn_log_get(id)
}

pub(crate) fn txn_log_insert<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    id: TransactionId,
    ts: Timestamp,
    committed: bool,
) -> Option<(Timestamp, bool)> {
    store.tapir_txn_log_insert(id, ts, committed)
}

pub(crate) fn txn_log_contains<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    id: &TransactionId,
) -> bool {
    store.tapir_txn_log_contains(id)
}

pub(crate) fn txn_log_len<K, V, T: TapirStoreBackend<K, V>>(store: &T) -> usize {
    store.tapir_txn_log_len()
}

pub(crate) fn raise_min_prepare_time<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    time: u64,
) -> u64 {
    store.tapir_raise_min_prepare_time(time)
}

pub(crate) fn finalize_min_prepare_time<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    time: u64,
) {
    store.tapir_finalize_min_prepare_time(time);
}

pub(crate) fn sync_min_prepare_time<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    time: u64,
) {
    store.tapir_sync_min_prepare_time(time);
}

pub(crate) fn reset_min_prepare_time_to_finalized<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
) {
    store.tapir_reset_min_prepare_time_to_finalized();
}

pub(crate) fn record_cdc_delta<K, V, T: TapirStoreBackend<K, V>>(
    store: &mut T,
    base_view: u64,
    delta: LeaderRecordDelta<K, V>,
) {
    store.tapir_record_cdc_delta(base_view, delta);
}

pub(crate) fn cdc_deltas_from<K, V, T: TapirStoreBackend<K, V>>(
    store: &T,
    from_view: u64,
) -> Vec<LeaderRecordDelta<K, V>> {
    store.tapir_cdc_deltas_from(from_view)
}

pub(crate) fn cdc_max_view<K, V, T: TapirStoreBackend<K, V>>(store: &T) -> Option<u64> {
    store.tapir_cdc_max_view()
}

#[cfg(test)]
#[derive(Clone)]
struct PendingCommittedTxn {
    txn_id: OccTransactionId,
    commit: Timestamp,
    read_set: Vec<(String, Timestamp)>,
    write_set: Vec<(String, Option<String>)>,
    scan_set: Vec<(String, String, Timestamp)>,
}

#[cfg(test)]
pub(crate) struct TapirStateRunner {
    tapir: TapirState<String, String, crate::mvcc::disk::memory_io::MemoryIo>,
    pending_commits: Vec<PendingCommittedTxn>,
    base_dir: PathBuf,
    io_flags: OpenFlags,
    manifest: UnifiedManifest,
}

#[cfg(test)]
impl TapirStateRunner {
    pub(crate) fn open(base_dir: PathBuf, io_flags: OpenFlags) -> Result<Self, StorageError> {
        crate::mvcc::disk::memory_io::MemoryIo::create_dir_all(&base_dir)?;
        let tapir = open::<String, String, crate::mvcc::disk::memory_io::MemoryIo>(&base_dir, io_flags)?;
        let manifest = UnifiedManifest::load::<crate::mvcc::disk::memory_io::MemoryIo>(&base_dir)?
            .unwrap_or_else(UnifiedManifest::new);
        Ok(Self {
            tapir,
            pending_commits: Vec::new(),
            base_dir,
            io_flags,
            manifest,
        })
    }

    pub(crate) fn list_dir_files(&self) -> Vec<(String, usize)> {
        let files = crate::mvcc::disk::memory_io::MemoryIo::list_files(&self.base_dir);
        let prefix = format!("{}/", self.base_dir.display());
        files
            .into_iter()
            .map(|(p, size)| {
                let path_string = p.to_string_lossy().to_string();
                let name = path_string
                    .strip_prefix(&prefix)
                    .unwrap_or(&path_string)
                    .to_string();
                (name, size)
            })
            .collect()
    }

    pub(crate) fn register_prepare(
        &mut self,
        txn_id: OccTransactionId,
        transaction: &crate::occ::Transaction<String, String, Timestamp>,
        commit_ts: Timestamp,
    ) {
        register_prepare(self, txn_id, transaction, commit_ts);
    }

    pub(crate) fn commit_prepared(
        &mut self,
        txn_id: OccTransactionId,
        commit: Timestamp,
    ) -> Result<(), StorageError> {
        commit_prepared(self, txn_id, commit)
    }

    pub(crate) fn seal(&mut self, min_view_vlog_size: u64) -> Result<(), StorageError> {
        let active_path = self
            .base_dir
            .join(format!("vlog_seg_{:04}.dat", self.manifest.active_segment_id));
        let mut active_vlog = UnifiedVlogSegment::<crate::mvcc::disk::memory_io::MemoryIo>::open_at(
            self.manifest.active_segment_id,
            active_path,
            self.manifest.active_write_offset,
            Vec::new(),
            self.io_flags,
        )?;
        active_vlog.start_view(self.manifest.current_view);

        let mut num_entries = 0_u32;
        for pending in self.pending_commits.drain(..) {
            let txn_ptr = active_vlog.append_committed_txn(
                pending.txn_id,
                pending.commit,
                &pending.read_set,
                &pending.write_set,
                &pending.scan_set,
            )?;
            num_entries += 1;
            self.tapir.index_committed_txn_ptr(pending.txn_id, txn_ptr);

            for (i, (key, value)) in pending.write_set.iter().enumerate() {
                let value_ref = if value.is_some() {
                    Some(ValueLocation::OnDisk(UnifiedVlogPrepareValuePtr {
                        txn_ptr,
                        write_index: i as u16,
                    }))
                } else {
                    None
                };
                self.tapir.unified_memtable_mut().insert(
                    key.clone(),
                    pending.commit,
                    UnifiedLsmEntry {
                        value_ref,
                        last_read_ts: None,
                    },
                );
            }

            for (key, read) in &pending.read_set {
                self.tapir
                    .unified_memtable_mut()
                    .update_last_read(key, *read, pending.commit.time);
            }
        }

        active_vlog.sync()?;
        active_vlog.finish_view(num_entries);

        let segment_size = active_vlog.write_offset();
        if segment_size >= min_view_vlog_size {
            let sealed_id = active_vlog.id;
            let sealed_path = active_vlog.path().clone();
            let sealed_views = active_vlog.views.clone();
            let sealed_size = active_vlog.write_offset();

            self.manifest.sealed_vlog_segments.push(VlogSegmentMeta {
                segment_id: sealed_id,
                path: sealed_path,
                views: sealed_views,
                total_size: sealed_size,
            });

            self.manifest.active_segment_id = self.manifest.next_segment_id;
            self.manifest.next_segment_id += 1;
            self.manifest.active_write_offset = 0;

            let new_path = self
                .base_dir
                .join(format!("vlog_seg_{:04}.dat", self.manifest.active_segment_id));
            let new_active = UnifiedVlogSegment::<crate::mvcc::disk::memory_io::MemoryIo>::open(
                self.manifest.active_segment_id,
                new_path,
                self.io_flags,
            )?;
            new_active.close();
        } else {
            self.manifest.active_segment_id = active_vlog.id;
            self.manifest.active_write_offset = active_vlog.write_offset();
        }

        self.manifest.current_view += 1;
        self.manifest
            .save::<crate::mvcc::disk::memory_io::MemoryIo>(&self.base_dir)?;
        active_vlog.close();

        finalize_after_seal(&mut self.tapir);
        Ok(())
    }

    pub(crate) fn get_at(
        &self,
        key: &str,
        ts: Timestamp,
    ) -> Result<(Option<String>, Timestamp), StorageError> {
        do_uncommitted_get_at(self, &key.to_string(), ts)
    }

    pub(crate) fn scan(
        &self,
        start: &str,
        end: &str,
        ts: Timestamp,
    ) -> Result<Vec<(String, Option<String>, Timestamp)>, StorageError> {
        do_uncommitted_scan(self, &start.to_string(), &end.to_string(), ts)
    }

    pub(crate) fn has_prepare_write_conflict(&self, key: &str) -> bool {
        self.tapir.has_prepare_write_conflict(&key.to_string())
    }

    pub(crate) fn contains_prepare(&self, txn_id: &OccTransactionId) -> bool {
        self.tapir.contains_prepare(txn_id)
    }
}

#[cfg(test)]
impl TapirStoreBackend<String, String> for TapirStateRunner {
    fn tapir_uncommitted_get(&self, key: &String) -> Result<(Option<String>, Timestamp), StorageError> {
        if let Some((ck, entry)) = self.tapir.unified_memtable().get_latest(key) {
            let ts = ck.timestamp.0;
            let value = resolve_value(self, entry)?;
            return Ok((value, ts));
        }
        Ok((None, Timestamp::default()))
    }

    fn tapir_uncommitted_get_at(
        &self,
        key: &String,
        ts: Timestamp,
    ) -> Result<(Option<String>, Timestamp), StorageError> {
        if let Some((ck, entry)) = self.tapir.unified_memtable().get_at(key, ts) {
            let write_ts = ck.timestamp.0;
            let value = resolve_value(self, entry)?;
            return Ok((value, write_ts));
        }
        Ok((None, Timestamp::default()))
    }

    fn tapir_uncommitted_scan(
        &self,
        start: &String,
        end: &String,
        ts: Timestamp,
    ) -> Result<Vec<(String, Option<String>, Timestamp)>, StorageError> {
        let mut out = Vec::new();
        for (ck, entry) in self.tapir.unified_memtable().scan(start, end, ts) {
            let value = resolve_value(self, entry)?;
            out.push((ck.key.clone(), value, ck.timestamp.0));
        }
        Ok(out)
    }

    fn tapir_txn_log_get(&self, id: &TransactionId) -> Option<(Timestamp, bool)> {
        self.tapir.txn_log_get(id)
    }

    fn tapir_txn_log_insert(
        &mut self,
        id: TransactionId,
        ts: Timestamp,
        committed: bool,
    ) -> Option<(Timestamp, bool)> {
        self.tapir.txn_log_insert(id, ts, committed)
    }

    fn tapir_txn_log_contains(&self, id: &TransactionId) -> bool {
        self.tapir.txn_log_contains(id)
    }

    fn tapir_txn_log_len(&self) -> usize {
        self.tapir.txn_log_len()
    }

    fn tapir_raise_min_prepare_time(&mut self, time: u64) -> u64 {
        self.tapir.raise_min_prepare_time(time)
    }

    fn tapir_finalize_min_prepare_time(&mut self, time: u64) {
        self.tapir.finalize_min_prepare_time(time)
    }

    fn tapir_sync_min_prepare_time(&mut self, time: u64) {
        self.tapir.sync_min_prepare_time(time)
    }

    fn tapir_reset_min_prepare_time_to_finalized(&mut self) {
        self.tapir.reset_min_prepare_time_to_finalized()
    }

    fn tapir_record_cdc_delta(&mut self, base_view: u64, delta: LeaderRecordDelta<String, String>) {
        self.tapir.record_cdc_delta(base_view, delta)
    }

    fn tapir_cdc_deltas_from(&self, from_view: u64) -> Vec<LeaderRecordDelta<String, String>> {
        self.tapir.cdc_deltas_from(from_view)
    }

    fn tapir_cdc_max_view(&self) -> Option<u64> {
        self.tapir.cdc_max_view()
    }

    fn tapir_append_committed_txn(
        &mut self,
        txn_id: OccTransactionId,
        commit: Timestamp,
        read_set: &[(String, Timestamp)],
        write_set: &[(String, Option<String>)],
        scan_set: &[(String, String, Timestamp)],
    ) -> Result<UnifiedVlogPtr, StorageError> {
        self.pending_commits.push(PendingCommittedTxn {
            txn_id,
            commit,
            read_set: read_set.to_vec(),
            write_set: write_set.to_vec(),
            scan_set: scan_set.to_vec(),
        });

        Ok(UnifiedVlogPtr {
            segment_id: u64::MAX,
            offset: self.pending_commits.len() as u64,
            length: 0,
        })
    }

    fn tapir_index_committed_txn_ptr(&mut self, txn_id: OccTransactionId, txn_ptr: UnifiedVlogPtr) {
        self.tapir.index_committed_txn_ptr(txn_id, txn_ptr);
    }

    fn tapir_register_prepare_raw(
        &mut self,
        txn_id: OccTransactionId,
        prepare: Arc<CachedPrepare<String, String>>,
    ) {
        self.tapir.register_prepare_raw(txn_id, prepare);
    }

    fn tapir_unregister_prepare(&mut self, txn_id: &OccTransactionId) {
        self.tapir.unregister_prepare(txn_id);
    }

    fn tapir_lookup_prepare(&self, txn_id: &OccTransactionId) -> Option<Arc<CachedPrepare<String, String>>> {
        self.tapir.lookup_prepare(txn_id)
    }

    fn tapir_lookup_prepare_value_ref(
        &self,
        txn_id: &OccTransactionId,
        write_index: u16,
    ) -> Option<(String, Option<String>)> {
        self.tapir.lookup_prepare_value_ref(txn_id, write_index)
    }

    fn tapir_lookup_committed_txn_ptr(&self, txn_id: &OccTransactionId) -> Option<UnifiedVlogPtr> {
        self.tapir.lookup_committed_txn_ptr(txn_id)
    }

    fn tapir_prepare_cache_get(
        &self,
        segment_id: u64,
        offset: u64,
    ) -> Option<Arc<CachedPrepare<String, String>>> {
        self.tapir.prepare_cache_get(segment_id, offset)
    }

    fn tapir_prepare_cache_insert(
        &self,
        segment_id: u64,
        offset: u64,
        prepare: Arc<CachedPrepare<String, String>>,
    ) {
        self.tapir.prepare_cache_insert(segment_id, offset, prepare);
    }

    fn tapir_increment_vlog_read_count(&self) {
        self.tapir.increment_vlog_read_count();
    }

    fn tapir_read_committed_txn(
        &self,
        txn_ptr: &UnifiedVlogPtr,
    ) -> Result<CachedPrepare<String, String>, StorageError> {
        if txn_ptr.segment_id == u64::MAX {
            return Err(StorageError::Codec("pending txn not sealed yet".to_string()));
        }

        let (segment_id, path, size, views) = if txn_ptr.segment_id == self.manifest.active_segment_id {
            (
                self.manifest.active_segment_id,
                self.base_dir
                    .join(format!("vlog_seg_{:04}.dat", self.manifest.active_segment_id)),
                self.manifest.active_write_offset,
                Vec::new(),
            )
        } else if let Some(meta) = self
            .manifest
            .sealed_vlog_segments
            .iter()
            .find(|m| m.segment_id == txn_ptr.segment_id)
        {
            (
                meta.segment_id,
                meta.path.clone(),
                meta.total_size,
                meta.views.clone(),
            )
        } else {
            return Err(StorageError::Codec(format!(
                "VLog segment {} not found",
                txn_ptr.segment_id
            )));
        };

        let segment = UnifiedVlogSegment::<crate::mvcc::disk::memory_io::MemoryIo>::open_at(
            segment_id,
            path,
            size,
            views,
            self.io_flags,
        )?;
        let prepare = segment.read_committed_txn(txn_ptr)?;
        segment.close();
        Ok(prepare)
    }

    fn tapir_insert_committed_write(
        &mut self,
        key: String,
        commit: Timestamp,
        value_ref: Option<ValueLocation>,
    ) {
        self.tapir.unified_memtable_mut().insert(
            key,
            commit,
            UnifiedLsmEntry {
                value_ref,
                last_read_ts: None,
            },
        );
    }

    fn tapir_update_last_read(&mut self, key: &String, read: Timestamp, commit_time: u64) {
        self.tapir
            .unified_memtable_mut()
            .update_last_read(key, read, commit_time);
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::IrClientId;

    #[test]
    fn resolve_on_disk_uses_cache_after_first_read() {
        let base_dir = std::path::PathBuf::from(format!(
            "/tapir-store-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system time should be after epoch")
                .as_nanos()
        ));
        let mut runner = TapirStateRunner::open(base_dir, OpenFlags::default())
            .expect("runner should open");

        let txn_id = OccTransactionId {
            client_id: IrClientId(7),
            number: 11,
        };
        let ts = Timestamp {
            time: 42,
            client_id: IrClientId(7),
        };

        commit_transaction_data(
            &mut runner,
            txn_id,
            &[("r".to_string(), ts)],
            &[("k".to_string(), Some("v".to_string()))],
            &[],
            ts,
        )
        .expect("commit should enqueue pending txn");
        runner.seal(0).expect("seal should flush pending txn to vlog");

        let txn_ptr = runner
            .tapir
            .lookup_committed_txn_ptr(&txn_id)
            .expect("committed txn ptr should exist after seal");
        let ptr = UnifiedVlogPrepareValuePtr {
            txn_ptr,
            write_index: 0,
        };

        let first = resolve_on_disk(&runner, &ptr).expect("first resolve should read from disk");
        assert_eq!(runner.tapir.vlog_read_count(), 1);
        assert_eq!(first.write_set[0].1.as_deref(), Some("v"));

        let second = resolve_on_disk(&runner, &ptr).expect("second resolve should hit cache");
        assert_eq!(runner.tapir.vlog_read_count(), 1);
        assert!(Arc::ptr_eq(&first, &second));
    }
}
