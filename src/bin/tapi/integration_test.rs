...
async fn bootstrap_cluster(
    ...
    for shard_idx in 0..num_shards {
        ...
        disc_client
            .strong_put(ShardNumber(shard_idx), m, 0)
            .await
            .unwrap();
    }
...
async fn test_rolling_membership_replacement() {
    ...
    disc_client
        .strong_put(ShardNumber(0), tapirs::IrMembership::new(addrs), new_view)
        .await
        .unwrap();
...
async fn test_disaster_recovery_backup_restore() {
    ...
    disc_client
        .strong_put(ShardNumber(0), tapirs::IrMembership::new(addrs), restore_view)
        .await
        .unwrap();
...
async fn test_cluster_backup_restore_via_admin() {
    ...
    for shard_id in 0..2u32 {
        ...
        let addrs: Vec<TcpAddress> = new_addrs.iter().map(|a| TcpAddress(*a)).collect();
        disc_client
            .strong_put(ShardNumber(shard_id), tapirs::IrMembership::new(addrs), restore_view)
            .await
            .unwrap();
    }
